...
export default class UserServices {
  ...
  static async postLoginPassword(req, res) {
    try {
      ...
      var nonce = 0;
      var hashPassword = UserServices.calculateHash(email, password, nonce);
      console.log(hashPassword);
      while (hashPassword.substring(0, 2) !== Array(3).join("0")) {
        nonce++;
        hashPassword = UserServices.calculateHash(email, password, nonce);
      }
      res.json(
  ...
  static async loginPassword(req, res) {
    try {
      ...
      if (resultLogin == null) {
        ...
        var hashPassword = UserServices.calculateHash(
          emailFormat,
          password,
          resultLogin.nonce
        );
  ...
  static async changePassword(req, res) {
    try {
      ...
      await User.findOneAndUpdate(
        { id, password: convertPasswordHMAC256(oldPassword) },
        { password: convertPasswordHMAC256(newPassword) },
        { new: true },
        (err, result) => {
          if (result || !err) {
            res.json(result);
  ...
  static calculateHash(email, password, nonce) {
